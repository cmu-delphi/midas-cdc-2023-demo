...
## Example: HHS hospitalization data
...
- `issue` is the date when the row started being valid (when it entered our database)


## Example: versioned HHS hospitalization data

- Here is what the same query looked like back in June ("as of" June 1st):
...
- a tibble with two required columns: `geo_value` and `time_value`
...
epi_df <- data %>% as_epi_df(
  geo_type = "state",
  time_type = "day",
  as_of = max(data$issue)
)
epi_df
```
...
data <- pub_covidcast(
  ...
  geo_values = "ca,fl,ny,tx",
  time_values = epirange(20230101, 20230601),
  issues = epirange(20230101, 20230601)
)
epi_archive <- data %>%
  select(
